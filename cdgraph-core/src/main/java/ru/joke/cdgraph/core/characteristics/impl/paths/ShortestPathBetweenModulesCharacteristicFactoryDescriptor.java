package ru.joke.cdgraph.core.characteristics.impl.paths;

import ru.joke.cdgraph.core.characteristics.CodeGraphCharacteristicFactoryDescriptor;

import java.lang.annotation.Retention;
import java.lang.annotation.RetentionPolicy;

/**
 * A descriptor of the factory of the characteristic that computes shortest path between
 * two modules of the graph.
 *
 * @author Alik
 *
 * @see ShortestPathBetweenModulesCharacteristic
 * @see ShortestPathBetweenModulesCharacteristicFactoryDescriptor
 */
@Retention(RetentionPolicy.RUNTIME)
@CodeGraphCharacteristicFactoryDescriptor(
        characteristicId = "shortest.path.between.modules",
        characteristicParametersType = PathBetweenModulesCharacteristicParameters.class
)
public @interface ShortestPathBetweenModulesCharacteristicFactoryDescriptor {
}
